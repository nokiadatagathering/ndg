...
package main.br.org.indt.ndg.controller.editor
{
    ...
    public class QuestionHelper
    {
        ...
        public function handleSkipLogicSelection(previousSelection:String):void
        {
            ...
            var orginCategory:XML = skipLogicOriginNode.parent();
            var skipToCategory:XML = selectedNode.parent();

            var orginCategoryId:String = orginCategory.@id;
            var skipToCategoryId:String = skipToCategory.@id;


            if ( skipToCategoryId != orginCategoryId ) {
                Alert.show( ConfigI18n.getInstance().getStringFile("editorResources", "SKIP_LOGIC_EXTERNAL_CATEGORY_WARNING"),
                            ConfigI18n.getInstance().getStringFile("editorResources", "SKIP_LOGIC"),
                            Alert.OK,
                            mainView);
                return;
            } else {
                if (  selectedNode.@id < skipLogicOriginNode.@id ) {
                    Alert.show( ConfigI18n.getInstance().getStringFile("editorResources", "SKIP_LOGIC_PREVIOUS_QUESTION_WARNING"),
                                ConfigI18n.getInstance().getStringFile("editorResources", "SKIP_LOGIC"),
                                Alert.OK,
                                mainView);
                    return;
                ...
                if ( selectedNode.@id == skipLogicOriginNode.@id ) {
                    Alert.show( ConfigI18n.getInstance().getStringFile("editorResources", "SKIP_LOGIC_SAME_QUESTION_WARNING"),
                                ConfigI18n.getInstance().getStringFile("editorResources", "SKIP_LOGIC"),
                                Alert.OK,
                                mainView);
                    return;
